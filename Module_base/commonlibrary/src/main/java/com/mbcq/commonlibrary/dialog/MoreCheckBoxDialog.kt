...
class MoreCheckBoxDialog : BaseDialogFragment {
    ...
    override fun initView(view: View, savedInstanceState: Bundle?) {
        ...
        val mCheckBoxOutViewAdapter = CheckBoxViewAdapter<BaseOutCheckedAdapterBean>(mContext)
        ...
        sure_btn.setOnClickListener(object : SingleClick() {
            override fun onSingleClick(v: View?) {
                ...
                for (item in mCheckBoxOutViewAdapter.getDatas()) {
                    if (item.checked) {
                        selectStr.append(item.title).append(" ")
