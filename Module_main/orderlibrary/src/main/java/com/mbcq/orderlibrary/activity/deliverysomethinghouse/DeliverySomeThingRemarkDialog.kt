...
//TODO 送货发布弹窗
class DeliverySomeThingRemarkDialog(var mScreenWidth: Int = 0, var mOlderDataJson: String = "", var mClick: OnClickInterface.OnClickInterface? = null) : BaseDialogFragment() {
    override fun setContentView(): Int = R.layout.dialog_delevery_somethig_house_remark
    ...
    override fun initView(view: View, savedInstanceState: Bundle?) {
        if (mOlderDataJson.isNotBlank()) {
            JSONObject(mOlderDataJson).also {
                ...
                actual_number_ed.setText(it.optString("qty"))//全部件数
            }

        }
        actual_number_ed.afterTextChanged {
            if (actual_number_ed.text.toString().isBlank()) return@afterTextChanged
            if (actual_number_ed.text.toString().toDouble() > JSONObject(mOlderDataJson).optString("qty").toInt()) {
