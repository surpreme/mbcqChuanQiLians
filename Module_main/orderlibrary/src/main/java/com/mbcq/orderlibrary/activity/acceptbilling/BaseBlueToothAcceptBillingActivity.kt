...
abstract class BaseBlueToothAcceptBillingActivity<V : BaseView, T : BasePresenterImpl<V>> : BaseAcceptBillingActivity<V, T>(), BaseView {
     ...
     * 远航物流托运单模板
     */
    protected fun print_YH_TYD_NEW1(kz: PrintBlueToothBean, isZhengzou: Boolean, showwebAddress: String, priceObj: JSONObject, zpSDK1: zpBluetoothPrinter) {
     ...
     * @param zpSDK1  sdk
     */
    protected fun print_LabelTemplated_XT423(kz: PrintBlueToothBean, num: Int, zpSDK1: zpBluetoothPrinter) {
        val mSimpleDateFormat = SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.getDefault())
        val font_bold = Typeface.create(Typeface.SERIF, Typeface.BOLD)
        val font = Typeface.SERIF
        val date: Date?
