...
import kotlinx.android.synthetic.main.activity_monthly_tuberculosis_sales.*
...
@Route(path = ARouterConstants.MonthlyTuberculosisSalesActivity)
class MonthlyTuberculosisSalesActivity : BaseSmartMVPActivity<MonthlyTuberculosisSalesContract.View, MonthlyTuberculosisSalesPresenter, MonthlyTuberculosisSalesBean>(), MonthlyTuberculosisSalesContract.View {
    var mStartDateTag = ""
    var mEndDateTag = ""
    var mShippingOutletsTag = ""
    var mSelectedType = 0
    
    override fun getLayoutId(): Int = R.layout.activity_monthly_tuberculosis_sales
...
//                        .setWidth(type_tv.width)
        ...
        generate_payment_voucher_btn.apply {
            onSingleClicks {
                ...
                ARouter.getInstance().build(ARouterConstants.MonthlyTuberculosisSalesPayCardActivity).withString("xSelectData", selectData).navigation()
            }
        }
        monthly_tuberculosis_sales_toolbar.setRightButtonOnClickListener(
                object : SingleClick() {
                    override fun onSingleClick(v: View?) {
                        WebDbUtil.getDbWebId(application, object : WebsDbInterface {
                            ...
                            override fun isSuccess(list: MutableList<WebAreaDbInfo>) {
                                FilterWithTimeDialog(getScreenWidth(), Gson().toJson(list), "webid", "webidCode", true, "月结核销记录筛选", true, mClickInterface = object : OnClickInterface.OnClickInterface {
        ...
        monthly_tuberculosis_sales_toolbar.setBackButtonOnClickListener(
                object : SingleClick() {
                    override fun onSingleClick(v: View?) {
                        onBackPressed()
                    }
    ...
    override fun getSmartLayoutId(): Int = R.id.monthly_tuberculosis_sales_smart
    override fun getSmartEmptyId(): Int = R.id.monthly_tuberculosis_sales_smart_frame
    override fun getRecyclerViewId(): Int = R.id.monthly_tuberculosis_sales_recycler

    override fun setAdapter(): BaseRecyclerAdapter<MonthlyTuberculosisSalesBean> = MonthlyTuberculosisSalesAdapter(mContext).also {
        it.mClickInterface = object : OnClickInterface.OnRecyclerClickInterface {
            override fun onItemClick(v: View, position: Int, mResult: String) {
                ARouter.getInstance().build(ARouterConstants.MonthlyTuberculosisSalesInfoActivity).withString("WriteoffInfo", mResult).navigation()
            }

        }
    }

    override fun getMonthlyS(list: List<MonthlyTuberculosisSalesBean>) {
        appendDatas(list)
    }
