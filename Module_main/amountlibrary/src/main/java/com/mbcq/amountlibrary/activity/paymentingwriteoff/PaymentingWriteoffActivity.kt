...
import kotlinx.android.synthetic.main.activity_paymenting_writeoff.*
...
@Route(path = ARouterConstants.PaymentingWriteoffActivity)
class PaymentingWriteoffActivity : BaseSmartMVPActivity<PaymentingWriteoffContract.View, PaymentingWriteoffPresenter, PaymentingWriteoffBean>(), PaymentingWriteoffContract.View {
    var mStartDateTag = ""
    var mEndDateTag = ""
    var mShippingOutletsTag = ""
    var mSelectedType = 0

    override fun getLayoutId(): Int = R.layout.activity_paymenting_writeoff
    ...
    override fun getPageDatas(mCurrentPage: Int) {
        ...
        mPresenter?.getPaymenting(mCurrentPage, mShippingOutletsTag, mStartDateTag, mEndDateTag, mSelectedType)
    }
...
//                        .setWidth(type_tv.width)
        ...
        generate_payment_voucher_btn.apply {
            onSingleClicks {
                ...
                ARouter.getInstance().build(ARouterConstants.PaymentingWriteOffPayCardActivity).withString("xSelectData", selectData).navigation()
            }
        }
        paymenting_write_off_toolbar.setRightButtonOnClickListener(object : SingleClick() {
            override fun onSingleClick(v: View?) {
                WebDbUtil.getDbWebId(application, object : WebsDbInterface {
                    ...
                    override fun isSuccess(list: MutableList<WebAreaDbInfo>) {
                        FilterWithTimeDialog(getScreenWidth(), Gson().toJson(list), "webid", "webidCode", true, "现付核销记录筛选", true, mClickInterface = object : OnClickInterface.OnClickInterface {
                            /**
                             * s1 网点
                             * s2  start@end
                             */
        ...
        paymenting_write_off_toolbar.setBackButtonOnClickListener(object : SingleClick() {
            override fun onSingleClick(v: View?) {
                onBackPressed()
            }
    ...
    override fun getSmartLayoutId(): Int = R.id.paymenting_write_off_smart
    override fun getSmartEmptyId(): Int = R.id.paymenting_write_off_smart_frame
    override fun getRecyclerViewId(): Int = R.id.paymenting_write_off_recycler

    override fun setAdapter(): BaseRecyclerAdapter<PaymentingWriteoffBean> = PaymentingWriteoffAdapter(mContext).also {
    ...
    override fun getPaymentingS(list: List<PaymentingWriteoffBean>) {
        appendDatas(list)
    }
