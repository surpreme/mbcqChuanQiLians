...
class CommonWriteOffReceivePayCardActivity : BaseMVPActivity<CommonWriteOffReceivePayCardContract.View, CommonWriteOffReceivePayCardPresenter>(), CommonWriteOffReceivePayCardContract.View {
    ...
    override fun getDocumentNoS(result: String) {
        ...
        if (!JSONObject(xSelectData).optString("billno").isNullOrBlank()){
            list.add(BaseTextAdapterBean("源  单 号：${JSONObject(xSelectData).optString("billno")}       金      额：${JSONObject(xSelectData).optString("accArrived")}\n摘      要：\n凭证编号：$result", ""))
        }else if (!JSONObject(xSelectData).optString("inoneVehicleFlag").isNullOrBlank()){
            list.add(BaseTextAdapterBean("车  次 号：${JSONObject(xSelectData).optString("inoneVehicleFlag")}       金      额：${JSONObject(xSelectData).optString("accArrived")}\n司机：${JSONObject(xSelectData).optString("chauffer")}       车牌号：${JSONObject(xSelectData).optString("vehicleNo")}\n摘      要：\n凭证编号：$result", ""))
        }
