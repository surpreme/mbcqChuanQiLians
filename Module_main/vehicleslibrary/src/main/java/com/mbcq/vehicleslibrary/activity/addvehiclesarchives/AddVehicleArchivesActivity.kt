...
class AddVehicleArchivesActivity : BaseMVPActivity<AddVehicleArchivesContract.View, AddVehicleArchivesPresenter>(), AddVehicleArchivesContract.View {
    ...
    fun saveVehicleInfo() {
        ...
        val BelWebCode = mCurrentWebIdId//所属网点编码
        obj.put("BelWebCode", BelWebCode)
        val BelWeb = mCurrentWebIdTag//所属网点编码对应的文字
        ...
        val IsraLiable = if (unreliable_cb.isChecked) 0 else 1//是否可靠0代表可靠1代表不可靠
    ...
    override fun saveInfoS(result: String) {
        TalkSureDialog(mContext, getScreenWidth(), "车辆档案${plate_number_ed.text}  建立完成，点击返回查看详情！") {
            onBackPressed()
        }.show()
