...
class FixHomeDeliveryConfigurationActivity : BaseMVPActivity<FixHomeDeliveryConfigurationContract.View, FixHomeDeliveryConfigurationPresenter>(), FixHomeDeliveryConfigurationContract.View {
    ...
    fun saveInfo() {
        ...
        mPresenter?.fixConfiguration(GsonUtils.toPrettyFormat(obj))
    }
    ...
    override fun fixConfiguration(result: String) {
        TalkSureDialog(mContext, getScreenWidth(), "上门提货配置已经修改成功，点击返回查看详情！") {
            onBackPressed()
        }.show()
    }
}